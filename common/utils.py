...
def load_data(data_dir):
    ...
    # The .reindex method is used to ensure that the DataFrame
    # has a consistent hourly frequency. The min(df["timestamp"])
    # and max(df["timestamp"]) lines define the date range for the
    # reindexing. The freq="H" parameter specifies that the index
    # should have hourly intervals.
    energy = energy.reindex(
        pd.date_range(min(energy["timestamp"]), max(energy["timestamp"]), freq="H")
    )

    # .drop("timestamp", axis=1) line removes the 'timestamp'
    # column from the DataFrame. This is done because the
    # timestamps are now embedded in the index.
